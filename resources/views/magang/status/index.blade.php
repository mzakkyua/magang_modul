                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($applications as $index => $app)
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-6 py-4">
                                    <span
                                        class="font-bold text-gray-800 block">{{ $app->vacancy->title ?? 'Posisi Terhapus' }}</span>
                                ...
                                <td class="px-6 py-4 text-center">
                                    @if ($app->status == 'accepted')
                                        <span
                                            class="bg-emerald-100 text-emerald-600 px-4 py-1.5 rounded-full text-xs font-bold uppercase border border-emerald-200">Telah
                                            Disetujui</span>
                                    @elseif($app->status == 'rejected')
                                        <span
                                            class="bg-red-100 text-red-600 px-4 py-1.5 rounded-full text-xs font-bold uppercase border border-red-200">Ditolak</span>
                                    @else
                                        <span
                                            class="bg-amber-100 text-amber-600 px-4 py-1.5 rounded-full text-xs font-bold uppercase border border-amber-200">Menunggu
                                            Review</span>
                                    @endif
                                </td>
                            </tr>
