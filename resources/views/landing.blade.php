...
<body class="bg-gray-50 font-sans text-gray-800">
                ...
                <div class="flex items-center space-x-4">
                    @auth
                        ...
                        <form action="{{ route('logout') }}" method="POST" class="inline">
                            @csrf
                            <button class="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-md text-sm font-medium">Logout</button>
                        </form>
                    @else
                        <a href="{{ route('login') }}" class="text-gray-700 hover:text-blue-600 font-medium">Masuk</a>
                        <a href="{{ route('register') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md text-sm font-medium">Daftar Magang</a>
                    @endauth
                </div>
            </div>
        </div>
    ...
    <div class="bg-blue-600 py-20 text-center text-white">
        ...
        <p class="text-lg md:text-xl text-blue-100 mb-8 max-w-2xl mx-auto">Bergabunglah dengan Dinas Tenaga Kerja untuk pengalaman magang yang nyata dan profesional.</p>
        
        <div class="max-w-xl mx-auto px-4">
            <form action="{{ route('landing.index') }}" method="GET" class="flex gap-2 bg-white p-2 rounded-lg shadow-lg">
                <input type="text" name="search" placeholder="Cari posisi magang..." class="flex-1 px-4 py-2 text-gray-800 outline-none rounded-md">
                <button type="submit" class="bg-blue-800 hover:bg-blue-900 text-white px-6 py-2 rounded-md font-semibold transition">
                    Cari
                </button>
            </form>
        </div>
                ...
                <div class="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-xl transition duration-300 border border-gray-100">
                    <div class="p-6">
                        ...
                        <div class="flex items-center gap-4 text-xs text-gray-500 mb-6">
                            <span class="flex items-center gap-1"><i class="bi bi-people"></i> Kuota: {{ $job->quota_slots }}</span>
                            <span class="flex items-center gap-1"><i class="bi bi-calendar"></i> {{ \Carbon\Carbon::parse($job->start_date)->format('d M') }} - {{ \Carbon\Carbon::parse($job->end_date)->format('d M Y') }}</span>
                        </div>

                        <a href="{{ route('landing.show', $job->id) }}" class="block w-full text-center bg-gray-900 hover:bg-gray-800 text-white py-2 rounded-lg font-medium transition">
                            Lihat Detail
                        </a>
                    </div>
                </div>
