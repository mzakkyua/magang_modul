...
@elseif(Auth::guard('magang')->check())
    <div class="flex items-center gap-10"> {{-- Container utama untuk menu + profile --}}
        ...
        <ul class="flex gap-8 items-center font-bold text-sm uppercase tracking-wider text-[#37517e]">
            <li>
                <a href="/" class="{{ request()->is('/') ? 'text-blue-600 border-b-2 border-blue-600' : '' }} hover:text-blue-600 transition pb-1">
                    Home
                </a>
            </li>
            <li>
                <a href="{{ request()->is('/') ? '#aboutus' : '/#aboutus' }}" class="hover:text-blue-600 transition pb-1">
                    Tentang Kami
                </a>
            </li>
            <li>
            ...
            <li>
                <a href="/sertifikat" class="{{ request()->is('sertifikat') ? 'text-blue-600 border-b-2 border-blue-600' : '' }} hover:text-blue-600 transition pb-1">
                    Sertifikat
                </a>
            </li>
        </ul>
    ...
    {{-- Tombol Pemicu --}}
    <div class="flex items-center gap-3 text-[#37517e] cursor-pointer px-3 py-2 rounded-lg transition-all duration-200">
        ...
        <i class="bi bi-chevron-down text-xs transition-transform duration-200 group-hover:rotate-180"></i>
    </div>
    ...
    <div class="absolute right-0 hidden group-hover:block pt-2 w-48 z-[9999]">
        <div class="bg-white border border-gray-100 rounded-xl shadow-xl py-2">
            ...
            <form action="{{ route('logout') }}" method="POST" class="w-full">
                @csrf
                <button type="submit" class="flex items-center gap-3 w-full px-4 py-2.5 text-sm text-red-600 hover:bg-red-50 transition text-left font-semibold">
