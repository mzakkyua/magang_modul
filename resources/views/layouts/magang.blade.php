    ...
    <header id="header" class="fixed top-0 w-full z-20 transition-all duration-300 bg-white shadow-md">
        <div class="container mx-auto px-4 py-3 flex items-center justify-between">
            ...
            <a href="/" class="flex items-center gap-3 group">
                <images src="{{ asset('') }}" alt="Logo"
                    class="max-h-14 group-hover:scale-105 transition-transform">
            </a>
            ...
            <nav class="hidden lg:flex items-center gap-8">
                ...
                <form action="{{ route('logout') }}" method="POST"
                    class="px-5 py-2 bg-[#37517e] text-white rounded-full text-sm font-medium hover:bg-blue-600 transition shadow-lg transform hover:-translate-y-0.5">
                    @csrf
        ...
        <div id="mobile-menu" class="hidden lg:hidden bg-white border-t absolute w-full left-0 shadow-lg">
            <ul class="flex flex-col p-4 gap-2 font-medium text-[#37517e]">
                ...
                <li><a href="/login" class="block px-4 py-2 bg-[#37517e] text-white rounded text-center mt-2">Login</a>
                </li>
            </ul>
        </div>
    </header>
