                ...
                <div class="hidden md:flex items-center space-x-3">
                    ...
                    @elseif(Auth::guard('magang')->check())
                        <div class="hidden md:flex items-center space-x-6">
                            ...
                            {{-- 1. Menu Navigasi --}}
                            <ul
                                class="flex gap-6 items-center font-bold text-[11px] uppercase tracking-widest text-[#37517e]">
                            ...
                            <div class="relative group border-l pl-4 ml-2" tabindex="0"> {{-- Tambahkan tabindex agar bisa menerima focus klik --}}
                                <button
                                    class="flex items-center gap-2 text-[#37517e] focus:outline-none py-1 px-2 rounded-lg hover:bg-gray-50 transition-all pointer-events-none">
                                    <div
                                        class="bg-blue-100 p-1 rounded-full flex items-center justify-center border border-blue-200">
                                    ...
                                    <i
                                        class="bi bi-chevron-down text-[9px] transition-transform group-hover:rotate-180 group-focus-within:rotate-180"></i>
                                </button>

                                {{-- Isi Menu Dropdown --}}
                                <div
                                    class="absolute right-0 w-40 pt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-9999"">

                                    {{-- Invisible Bridge --}}
                                    <div class="h-2 w-full bg-transparent"></div>

                                    <div
                                        class="bg-white border border-gray-100 rounded-xl shadow-2xl py-2 overflow-hidden ring-1 ring-black ring-opacity-5">
                                        <a href="{{ route('profile.edit') }}"
                                            class="flex items-center gap-3 px-4 py-2.5 text-[12px] font-bold text-gray-700 hover:bg-blue-50 hover:text-blue-600 transition">
                                        ...
                                        <form action="{{ route('logout') }}" method="POST" class="m-0">
                                            @csrf
                                            <button type="submit"
                                                class="flex items-center gap-3 w-full px-4 py-2.5 text-[12px] font-bold text-red-600 hover:bg-red-50 transition text-left">
                    ...
                    @else
                        <div class="flex gap-3">
                            <a href="{{ route('login') }}"
                                class="bg-white text-blue-600 border border-blue-600 px-6 py-2 rounded-full font-bold hover:bg-blue-50 transition">Masuk</a>
                            <a href="{{ route('register') }}"
                                class="bg-blue-600 text-white px-6 py-2 rounded-full font-bold hover:bg-blue-700 transition shadow-lg shadow-blue-600/30">Daftar</a>
                        </div>
                    @endif
                </div>
            </div>
